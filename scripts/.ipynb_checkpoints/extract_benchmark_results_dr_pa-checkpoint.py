...
def count_jargon(transcript: list) -> float:
    """Average number of medical jargon words found per patient turn."""
...
def count_questions(transcript: list) -> int:
    """Count doctor turns that contain a question mark."""
...
def avg_word_count(transcript: list, role: str) -> float:
    """Average word count for a given role."""
    turns = [m for m in transcript if m.get("role") == role]
    if not turns:
        return 0.0
    return round(sum(len(m.get("content", "").split()) for m in turns) / len(turns), 1)


def turns_completed(transcript: list) -> int:
    """Number of patient turns completed."""
    return sum(1 for m in transcript if m.get("role") == "Patient")


def extract_llm_judge(data: dict, role: str) -> dict:
    """
    Extract LLM judge sub-scores for 'patient' or 'doctor'.
    Returns a flat dict of score fields, or empty strings on missing/error.
    """
    key   = f"llm_judge_{role}"
    block = data.get(key)

    # If judge was skipped (--no-judge) or JSON parsing failed
...
def extract_results():
    ...
    for patient_key in MODELS:
        for doctor_key in MODELS:
            folder   = os.path.join(RESULTS_DIR, f"patient_{patient_key}_doctor_{doctor_key}")
            filepath = os.path.join(folder, "transcript.json")

            if not os.path.exists(filepath):
                print(f"⚠️  Missing: patient={patient_key.upper()} | doctor={doctor_key.upper()} — skipping.")
                continue
            ...
            patient_upper = patient_key.upper()
            doctor_upper  = doctor_key.upper()

            # --- Core columns ---
            row = {
                "Config":        f"{patient_upper} × {doctor_upper}",
                "Patient Model": patient_upper,
                "Doctor Model":  doctor_upper,
            ...
            # --- LLM judge sub-scores (patient + doctor) ---
            row.update(extract_llm_judge(data, "patient"))
            row.update(extract_llm_judge(data, "doctor"))

            csv_data.append(row)
            h_total = row["Heuristic Total (/20)"]
            l_total = row["LLM Judge Total (/20)"]
            print(f"✅ patient={patient_upper:12s} | doctor={doctor_upper:12s} | "
                  f"Heuristic={h_total}/20 | LLM={l_total}/20")

    if not csv_data:
        print("❌ No data found. Run run_benchmark_simulation.py first.")
        return
